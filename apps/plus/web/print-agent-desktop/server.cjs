...
app.post('/print/zpl', async (req, res) => { 
  try { 
    let { ip, port, zpl, profile, useCustomFont = false } = req.body || {}; 
    ...
    if (!port) port = 9100; 
    if (!ip || !zpl) return res.status(400).json({ error: 'Campos obrigatórios ausentes: ip/zpl' }); 
    
    // Se usar fonte customizada, adicionar comandos de fonte antes do ZPL
    if (useCustomFont) {
      const customFontCommands = '^XA^CWZ,E:MANROPE.TTF^CI28^FS'
      zpl = customFontCommands + zpl
    }
...
app.get('/', (req, res) => {
  // No Electron, usar process.resourcesPath para acessar recursos empacotados
