...
export default function EtiquetasProdutosPage() {
  ...
  const [etiquetas, setEtiquetas] = useState<EtiquetaProduto[]>([]);
  const [busca, setBusca] = useState('');
  const [etiquetasFiltradas, setEtiquetasFiltradas] = useState<EtiquetaProduto[]>([]);
  const [selectedEtiqueta, setSelectedEtiqueta] = useState<EtiquetaProduto | null>(null);

  useEffect(() => {
    // Carregar etiquetas do localStorage
    const etiquetasSalvas = localStorage.getItem('etiquetasProdutos');
    if (etiquetasSalvas) {
    ...
    if (etiquetaId) {
      ...
      if (etiquetasSalvas) {
        const etiquetasData = JSON.parse(etiquetasSalvas);
        const etiquetaEncontrada = etiquetasData.find((e: EtiquetaProduto) => e.id === etiquetaId);
  ...
  const handleImprimir = (etiqueta: EtiquetaProduto) => {
  ...
  const handleEditar = (etiqueta: EtiquetaProduto) => {
  ...
  const handleExcluir = (id: string) => {
    if (confirm('Tem certeza que deseja excluir esta etiqueta?')) {
      ...
      localStorage.setItem('etiquetasProdutos', JSON.stringify(etiquetasAtualizadas));
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">Etiquetas - Produtos</h1>
                  <p className="mt-2 text-gray-600">Gerencie etiquetas para produtos finais e acabados</p>
                </div>
              </div>
              <button
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-600">
                      Categoria: {selectedEtiqueta.categoria} | Quantidade: {selectedEtiqueta.quantidade} {selectedEtiqueta.unidade} | Preço: R$ {selectedEtiqueta.preco.toFixed(2)}
                    </p>
                  </div>
                </div>
