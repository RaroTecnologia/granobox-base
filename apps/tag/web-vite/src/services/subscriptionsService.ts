...
class SubscriptionsService {
  ...
  async getSubscriptionByClient(clientId: string): Promise<Subscription[]> {
    const response = await api.get<Subscription[]>(`/subscriptions/client/${clientId}`);
    return response.data;
  }

  async getCurrentSubscription(clientId: string): Promise<Subscription | null> {
    const response = await api.get<Subscription>(`/subscriptions/current/${clientId}`);
    return response.data;
  }
  ...
  async reactivateSubscription(id: string): Promise<Subscription> {
    const response = await api.patch<Subscription>(`/subscriptions/${id}/reactivate`);
    return response.data;
  }
