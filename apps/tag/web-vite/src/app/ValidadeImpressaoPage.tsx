...
import { Printer, ArrowLeft, Trash, CheckCircle, Clock, House, Thermometer, Snowflake, Square, Warning, Tag } from '@phosphor-icons/react'
...
interface ItemFila {
  ...
  peso: string
  unidade: string
  conservacao: 'ambiente' | 'refrigerado' | 'congelado'
  dataValidade: string
  timestamp: Date
}
...
export default function ValidadeImpressaoPage() {
  ...
  useEffect(() => {
    const fila = JSON.parse(localStorage.getItem('filaValidadeEtiquetas') || '[]')
  ...
  const salvarFila = (novaFila: ItemFila[]) => {
    ...
    localStorage.setItem('filaValidadeEtiquetas', JSON.stringify(novaFila))
  }
          ...
          {filaItens.length === 0 ? (
            ...
            <>
              {filaItens.map((item, index) => {
                const conservacaoInfo = getConservacaoInfo(item.conservacao)
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-1 mt-1">
                          ...
                          <span className={`text-xs ${theme === 'dark' ? 'text-dark-400' : 'text-dark-600'}`}>
                            {conservacaoInfo.label} - Validade: {item.dataValidade}
                          </span>
                        </div>
                      </div>
